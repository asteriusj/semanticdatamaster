...
function isURL(uri) {
	console.log('isURL uri', uri)
	let rs = false;
	if (uri.startsWith("http")) {
		rs = new RegExp(hardcoded.xsd$anyURI.pattern).test(uri);
	}
	console.log('isURL rs', rs)
	return rs
}
...
function constructor(options) {
	console.log('index constructor options', options)
	this._externals = {};
	this.cb = function () {};
	this.uri = '';
	this.string = '';
	...
	// console.log('index constructor this')
	//console.log(this);
}

constructor.prototype.parse = function (v, cb) {
	console.log('index parse v :', v)
	if (cb) this.cb = cb;
	if (isURL(v)) {
		this.uri = v;
		...
		console.log('is URL this.options', this.options)
		...
		// console.log('is not URL this', this)
		//
		console.log('is not URL v', v)
		this.options.uri = v
		console.log('is not URL this.options', this.options)
		// Actually nes to get xml doc contents at v
		var filepath = this.options.uri
		fs.readFile(filepath, function (err, body) {
			if (err) {
				...
				// console.log('is not URL json', json)
				// TODO err handling ...
		...
		// console.log('is not URL xstuff', xstuff)
...
constructor.prototype.parseURL = function () {
	console.log('prototype.parseURL this:', this)
	...
	console.log('prototype.parseURL protocol _headers:', protocol, _headers)
	request({
		url: this.uri,
		headers: _headers
...
constructor.prototype.result = function (err, res, body) {
	...
	if (res.statusCode == 200) {
		...
		console.log('prototype.result json :', json)
		return this.cb(null, json);
	}
}

constructor.prototype.parseFile = function () {
	console.log('constructor.prototype.parseFile callerDir uri', this.options.callerDir, this.uri)
	if (this.options.callerDir) this.uri = path.join(this.options.callerDir, this.uri);
	console.log('constructor.prototype.parseFile this.uri', this.uri)
	var filepath = this.uri
	fs.readFile(filepath, function (err, body) {
...
constructor.prototype.parseOWL = function (owlXML) {
	console.log('parseOWL owlXML:', owlXML)
	...
	if (!(c._rdf in o) || !(c._class in o[c._rdf])) {
		/*return*/
		console.warn('Did not find any OWL specifications. ');
	}

	console.log('parseOWL options', options)
	var DIR = path.join(this.options.callerDir, this.options.outputDir, defaultPath(this.base));
	console.log('parseOWL DIR', DIR)
