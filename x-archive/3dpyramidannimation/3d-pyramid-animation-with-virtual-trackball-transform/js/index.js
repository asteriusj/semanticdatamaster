var WIDTH = 160,
	...
	LENGTH = 140,
	EXPAND = 100,
	IMG_FRONT = 'https://ZulNs.github.io/images/pyramid/front.png',
	IMG_BACK = 'https://ZulNs.github.io/images/pyramid/back.png',
	// IMG_LEFT = 'https://ZulNs.github.io/images/pyramid/left.png',
	IMG_RIGHT = 'https://ZulNs.github.io/images/pyramid/right.png',
	IMG_BOTTOM = 'https://ZulNs.github.io/images/pyramid/bottom.png',
	BACKFACE_FB = 'https://ZulNs.github.io/images/pyramid/bffb.png',
	BACKFACE_LR = 'https://ZulNs.github.io/images/pyramid/bflr.png',
	BACKFACE_B = 'https://ZulNs.github.io/images/pyramid/bfb.png',
	NAME = 'pyramid',
	WRAP = NAME + '-wrapper',
	_cssRules = '',
	_frontTH,
...
// 	console.log('createSide side',side)
// 	return side;
// }

function createTriSide(w, h, bg, bx, by, cname) {
	var side = document.createElement("div"),
		cssText =
			'width: ' + w.toFixed(2) + 'px;' +
			'height: ' + ( h.toFixed(2) ) + 'px;' +
	...
	side.className = cname;
	side.style.cssText = cssText;
	console.log('createTriSide side',side)
	return side;
}
...
// 	console.log('createFace face',face)
// 	return face;
// }

function createTriFace(w, h, rx, ry, ty, tz, rx2, urx, img, ix, iy, bf, bfx, bfy, cname) {
	var face = document.createElement("div"),
		css,
		cssText =
			'width: ' + w.toFixed(2) + 'px;' +
			...
			'margin-left: ' + (-w / 2).toFixed(2) + 'px;' +
			'margin-top: ' + (-h / 2).toFixed(2) + 'px;',
		front = createTriSide(w, h, img, ix, iy, 'frontface');
	css = 'transform: rotateX(' + rx.toFixed(2) + 'deg) rotateY(' + ry.toFixed(2) + 'deg) translate3d(0px, ' +
			ty.toFixed(2) + 'px, ' + tz.toFixed(2) + 'px) rotateX(' + rx2.toFixed(2) + 'deg)';
	cssText += addVendorPrefix(css + ';');
	css += ' translateZ(' + EXPAND + 'px) !important;'
	...
	face.classList.add(cname);
	face.style.cssText = cssText;
	face.appendChild(createTriSide(w, h, bf, bfx, bfy, 'backface'));
	front.innerHTML = cname;
	face.appendChild(front);
	console.log('createTriFace face',face)
	return face;
}
...
function createModel() {
	...
	frx = frx * 180 / Math.PI;
	srx = srx * 180 / Math.PI;
	model.appendChild(createTriFace(WIDTH, _frontTH, 0, 179.99, -fdh, LENGTH / 2, frx, -90, IMG_BACK, 0, 0, BACKFACE_FB, 0, 0, 'back'));
	// model.appendChild(createFace(LENGTH, _sideTH, 0, -90, -sdh, WIDTH / 2, srx, -90, IMG_LEFT, 0, 0, BACKFACE_LR, 0, 0, 'left'));
	model.appendChild(createTriFace(LENGTH, _sideTH, 0, 90, -sdh, WIDTH / 2, srx, -90, IMG_RIGHT, 0, 0, BACKFACE_LR, 0, 0, 'right'));
	// model.appendChild(createFace(WIDTH, LENGTH, -90, 0, 0, HEIGHT / 2, 0, 0, IMG_BOTTOM, 0, 0, BACKFACE_B, 0, 0, 'bottom'));
	model.appendChild(createTriFace(WIDTH, _frontTH, 0, 0, -fdh, LENGTH / 2, frx, -90, IMG_FRONT, 0, 0, BACKFACE_FB, 0, 0, 'front'));

	model.appendChild(createTriFace(WIDTH, LENGTH, -90, 0, 0, HEIGHT / 2, 0, 0, IMG_BOTTOM, 0, 0, BACKFACE_B, 0, 0, 'bottom'));
